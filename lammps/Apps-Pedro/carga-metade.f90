 ...
 !     write(6,'(a)')'Enter name of file to convert'
  ...
  !    write(6,'(a)')'Enter number of atoms in MD cell'
   ...
   !   write(6,'(a)')'Number of configurations'
      ...
      norma3=0
      norma = 0
        open (25,file='meia-carga-on-electrodes.dat', status='unknown')
1000  format(i5,2f8.3,4i5)
      ...
      do j=1,nconfig
        ...
        a = natms - 1367 ! Numero do atomo que quero com menor z
        do i=1,natms 
          read(7,*)k,kk,atomtype,kkk,qq(i),xxx(i),yyy(i),zzz(i)
          if (j.gt.start) then
            if (atomtype.eq.'T') then
                ...
                if(zzz(i).gt.0) then
                   zzz(i) = zzz(i)-b
         !               b=39+15.5
                        if(zzz(i).ge.0.and.zzz(i).le.15.5) then
                        chneg=chneg+qq(i)
                        else
                        chpos=chpos+qq(i)
                        endif
