program z_density
 ...
 integer:: nconfig
 real:: boxx,boxy,boxz
 integer:: maxbin, bin1, bin2, bin3, i, j, bin, k, kk, kkk 
...
!      write(6,'(a)')'Number of configurations'
 ...
 close (5)
      ...
      do j=1,nconfig
        ...
        do i=1,natms 
          read(7,*)atmnam(i),k,kk,qq(i),xxx(i),yyy(i),zzz(i)
          kk=i
          zzz(i)=zzz(i)+ 70.
          if (j.ge.start) then
              bin=int(zzz(i)/delr)
              if(bin.le.maxbin) then
                histz1(bin) = histz1(bin)+qq(i)
              endif
          endif
        enddo
1000 format (f8.4,4f10.6)       
        ...
        open (25,file='total-charge-density-z.dat', status='unknown')
        incr=0
        vnorm=boxxh*boxyh*delr*norma
        write(*,*)boxxh,boxyh,delr,norma,vnorm
        do bin=1,maxbin
          write(25,1000) incr,histz1(bin)/vnorm
          incr = incr + delr
        enddo
        close(25)
      enddo
